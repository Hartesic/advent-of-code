...
def part_one():
    ...
    for char in my_input:
        expected_char = (expected_chars[expected_char_idx]
                         if expected_char_idx < len(expected_chars)
                         else None)
        if (expected_char is None
                and char.isdigit()
                and len(current_param) < max_param_len):
            current_param += char
        elif (expected_char is None
              and char == param_separator_char
              and factor is None):
            factor = int(current_param)
            current_param = ''
        elif expected_char is None and char == param_end_char:
            products.append(factor * int(current_param))
            expected_char_idx = 0
            current_param = ''
            factor = None
...
print('Valid memory multiplications sum:', part_one())
...
def part_two():
    param_separator_char = ','
    instruction_end_char = ')'
    mul_chars = ['m', 'u', 'l', '(']
    max_param_len = 3
    mul_enable_chars = ['d', 'o', '(']
    mul_disable_chars = ['d', 'o', 'n', '\'', 't', '(']

    products = []

    current_instruction = None
    instruction_char_idx = 0
    current_param = ''
    factor = None
    is_mul_enabled = True
    ...
    for char in my_input:
        if current_instruction is None:
            if (is_mul_enabled
                    and char == mul_disable_chars[0]):
                current_instruction = mul_disable_chars
                instruction_char_idx = 1
                continue
            if (not is_mul_enabled
                    and char == mul_enable_chars[0]):
                current_instruction = mul_enable_chars
                instruction_char_idx = 1
                continue

        if current_instruction is not None:
            if (instruction_char_idx < len(current_instruction)
                    and char == current_instruction[instruction_char_idx]):
                instruction_char_idx += 1
                continue
            if (instruction_char_idx == len(current_instruction)
                    and char == instruction_end_char):
                is_mul_enabled = (True
                                  if current_instruction is mul_enable_chars
                                  else False)
            instruction_char_idx = 0
            current_instruction = None
        ...
        expected_char = (mul_chars[instruction_char_idx]
                         if instruction_char_idx < len(mul_chars)
                         else None)
        if (expected_char is None
                and char.isdigit()
                and len(current_param) < max_param_len):
            ...
            factor = int(current_param)
            current_param = ''
        elif expected_char is None and char == instruction_end_char:
            products.append(factor * int(current_param))
            instruction_char_idx = 0
            current_param = ''
            factor = None
...
print('Valid and enabled memory multiplications sum:', part_two())
